        ...
        stage('Push to ECR') {
            steps {
                withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-credentials']]) {
                                        bat '''
                                            aws ecr create-repository --repository-name weathernow-frontend --region %AWS_DEFAULT_REGION% || ver>nul
                                            aws ecr create-repository --repository-name weathernow-backend --region %AWS_DEFAULT_REGION% || ver>nul
            ...
            steps {
                withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-credentials']]) {
                                        bat '''
                                            ...
                                            aws ecs register-task-definition --cli-input-json file://ecs/backend-taskdef.json --region %AWS_DEFAULT_REGION%
                                            aws ecs register-task-definition --cli-input-json file://ecs/frontend-taskdef.json --region %AWS_DEFAULT_REGION%
                                        '''
                }
            }
        }
            ...
            steps {
                withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-credentials']]) {
                                        bat '''
                                            ...
                                            aws ecs update-service --cluster weathernow-cluster --service weathernow-backend-service --force-new-deployment --region %AWS_DEFAULT_REGION% || ver>nul
                                            aws ecs update-service --cluster weathernow-cluster --service weathernow-frontend-service --force-new-deployment --region %AWS_DEFAULT_REGION% || ver>nul
                                        '''
                }
            }
        }
